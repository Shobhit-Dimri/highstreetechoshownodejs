...
// [START permissions]
exports.getNameFunction = functions.https.onRequest((request, response) => {
  ...
  function requestPermission (app, permission, firebaseKey, speechCallback) {
    ...
    return new Promise(function (resolve, reject) {
	  ...
	  console.log('#### User Token: ' + app.getUser().accessToken);
      firebaseAdmin.database().ref('users/' + encodeAsFirebaseKey(userId))
        .once('value', function (data) {
          if (data && data.val() && data.val()[firebaseKey]) {
			console.log('### Resolve - Tell Output. FirebaseKey - '+firebaseKey);
			console.log('### data.val()[firebaseKey] - '+data.val()[firebaseKey]);
            //let speechOutput = speechCallback(data.val()[firebaseKey]);
            //resolve(app.tell(speechOutput));
			console.log('### Still asking for permission...');
			resolve(app.askForPermission('Sure I can help you buy that. ', permission));
          } else {
			console.log('### Resolve - ask for Permission. FirebaseKey - '+firebaseKey);
            resolve(app.askForPermission('Sure I can help you buy that. ', permission));
          }
        });
    });
  ...
  console.log('$$$$ Read Mind');
  console.log('$$$$ User Name: ' + app.getUserName().displayName);
  console.log('$$$$ User Id: ' + app.getUser().user_id);
    if (app.isPermissionGranted()) {
      ...
      if (permission === app.SupportedPermissions.NAME) {
		console.log('$$$$ Read Mind - Permision = Name');
  ...
  function readMindResponse (app, userData, firebaseKey, speechCallback) {
    let userId = app.getUser().user_id;
    console.log('@@@@ Read Mind Response');
